...
/*
 * 为了避免GPU写操作还没有开始就已经对某个资源R按顺序先写后读，产生资源冒险，需要将资源设置为合理的状态转换
 */
void D3DApp::FlushCommandQueue() { // 使用Fence强制令CPU等待，直到GPU完成所有命令,
...
// 每个gpu都至少维护了一个命令队列（本质是避免频繁的内存创建取消、分配。内存一直只用了一块的环形缓冲区），透过命令列表提交命令
...
void D3DApp::LogAdapters() { // 使用IDXGIFactory创建IDXGISwapChain并枚举显示适配器；显示适配器用IDXGIAdapter表示
	UINT i = 0;
...
void D3DApp::LogOutputDisplayModes(IDXGIOutput* output, DXGI_FORMAT format) const { // 获取显示输出对此格式支持的全部显示模式
	UINT count = 0;
	UINT flag = 0;
